<template>
  <div class="flex h-screen bg-gray-100 dark:bg-gray-950">
    ...
    <aside class="w-56 bg-gray-900 text-white flex flex-col">
      ...
      <div class="p-4">
        <div class="flex items-center space-x-2">
          ...
          <span class="text-lg font-bold">deliverytrack</span>
        </div>
      </div>
      ...
      <div class="border-t border-gray-800 p-3">
        <div class="flex items-center space-x-3 px-3 py-2">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-white truncate">Ma Boutique</p>
            <p class="text-xs text-gray-400 truncate">admin@maboutique.tn</p>
          </div>
        </div>
      </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div v-for="category in filteredTaskCategories" :key="category.id" class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
              ...
              <div class="divide-y divide-gray-100 dark:divide-gray-800">
                <div v-for="task in category.tasks" :key="task.id"
                  ...
                  <div class="flex items-center space-x-2">
                    <!-- Quick Action Button -->
                    <button v-if="task.action && !task.completed" @click="executeTaskAction(task)" class="btn-cta btn-cta-sm flex items-center space-x-1">
        ...
        <header class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-sm text-gray-500 mt-1">{{ delayedShipments.length }} colis nécessitent votre attention</p>
            </div>
            <div class="flex items-center space-x-3">
              ...
              <button class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg text-sm font-medium transition-colors flex items-center space-x-2">
                <PhoneIcon class="w-4 h-4" />
                <span>Relancer tous</span>
              </button>
            </div>
          </div>
        </header>
        <main class="flex-1 overflow-y-auto p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <button @click="delayedFilter = 'all'" :class="['rounded-xl p-4 border transition-colors text-left', delayedFilter === 'all' ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800']">
              <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ delayedShipments.length }}</p>
            ...
            <button @click="delayedFilter = '24h'" :class="['rounded-xl p-4 border transition-colors text-left', delayedFilter === '24h' ? 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800']">
              <p class="text-2xl font-bold text-yellow-600">{{ delayedShipments.filter(s => s.delayDays === 1).length }}</p>
            ...
            <button @click="delayedFilter = '48h'" :class="['rounded-xl p-4 border transition-colors text-left', delayedFilter === '48h' ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800']">
              <p class="text-2xl font-bold text-orange-600">{{ delayedShipments.filter(s => s.delayDays >= 2 && s.delayDays <= 3).length }}</p>
            ...
            <button @click="delayedFilter = 'critical'" :class="['rounded-xl p-4 border transition-colors text-left', delayedFilter === 'critical' ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800']">
              <p class="text-2xl font-bold text-red-600">{{ delayedShipments.filter(s => s.delayDays > 3).length }}</p>
          ...
          <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
            ...
            <div class="divide-y divide-gray-100 dark:divide-gray-800">
              <div v-for="shipment in filteredDelayedShipments" :key="shipment.id" class="px-5 py-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                <div class="flex items-center justify-between">
                  <div class="flex items-center space-x-4">
                    <input type="checkbox" class="w-4 h-4 text-orange-500 rounded border-gray-300 focus:ring-orange-500" />
                    <div :class="[
                  ...
                  <div class="flex items-center space-x-2">
                    <button class="p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Appeler client">
                      <PhoneIcon class="w-4 h-4" />
                    </button>
      ...
      <template v-if="activeSection === 'returns-alerts'">
        <header class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4">
          <div class="flex items-center justify-between">
            ...
            <button class="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg text-sm font-medium transition-colors flex items-center space-x-2">
              <MessageCircle class="w-4 h-4" />
              <span>Envoyer SMS groupé</span>
            </button>
          </div>
        </header>
        <main class="flex-1 overflow-y-auto p-6">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl p-4 border border-gray-200 dark:border-gray-800">
              <div class="flex items-center space-x-3">
                <div class="w-10 h-10 bg-yellow-100 dark:bg-yellow-900/30 rounded-lg flex items-center justify-center">
                  <PhoneIcon class="w-5 h-5 text-yellow-600" />
                </div>
                <div>
              ...
              <div v-for="alert in returnAlerts.filter(a => a.type === 'failed-attempt')" :key="alert.id" class="px-5 py-4">
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center space-x-2">
                    ...
                    <button class="px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium rounded-lg transition-colors flex items-center space-x-1">
            ...
            <div class="divide-y divide-gray-100 dark:divide-gray-800">
              <div v-for="alert in returnAlerts.filter(a => a.type === 'unreachable')" :key="alert.id" class="px-5 py-4">
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center space-x-2">
                    <button class="px-3 py-1.5 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 text-xs font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                      Envoyer SMS
                    </button>
                    <button class="btn-cta btn-cta-sm">
                      Réessayer
        ...
        <main class="flex-1 overflow-y-auto p-6">
          <div class="max-w-3xl mx-auto space-y-6">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
              <div class="px-6 py-4 bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-800">
                <div class="flex items-center space-x-3">
                  ...
                  <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Expédition</h2>
                </div>
              </div>
              <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Numéro de suivi</label>
                    <input v-model="newShipment.trackingNumber" type="text" placeholder="Ex: YAL-2024-001234" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Transporteur</label>
                    <select v-model="newShipment.carrier" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
                      <option value="">Sélectionner un transporteur</option>
                      <option value="Aramex">Aramex</option>
                      <option value="Rapid Poste">Rapid Poste</option>
                      <option value="STL Express">STL Express</option>
                      <option value="Sotumag">Sotumag</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              <div class="p-6 space-y-4">
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Nom destinataire <span class="text-red-500">*</span>
                  </label>
                  <input v-model="newShipment.customerName" type="text" placeholder="Entrez le nom du destinataire" class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
                </div>
              ...
              <div class="p-6 space-y-4">
                ...
                <div class="flex flex-col md:flex-row md:items-end gap-4">
                  ...
                  <div class="flex items-center">
                    <label class="flex items-center space-x-2 cursor-pointer px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                      <input type="checkbox" v-model="newShipment.isFragile" class="w-4 h-4 text-orange-500 border-gray-300 rounded focus:ring-orange-500" />
                ...
                <div class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
                  ...
                  <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between">
                      <label class="text-sm text-gray-600 dark:text-gray-400">Frais livraison</label>
                      <div class="flex items-center space-x-2">
            ...
            <div class="flex items-center justify-end space-x-3 pb-6">
              ...
              <button @click="addShipment" class="btn-cta btn-cta-sm">
                Créer le colis
              </button>
            </div>
          </div>
        ...
        <main class="flex-1 overflow-y-auto p-6">
          ...
          <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
            ...
            <div class="grid grid-cols-12 gap-4 px-6 py-3 bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-800 text-xs font-semibold text-gray-500 uppercase tracking-wider">
              ...
              <div class="col-span-1 text-center">Emails sent</div>
              <div class="col-span-1 text-center">SMS sent</div>
            ...
            <div v-for="(flow, index) in notificationFlows" :key="index" class="grid grid-cols-12 gap-4 px-6 py-4 border-b border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors items-center">
              ...
              <!-- SMS Sent -->
              <div class="col-span-1 text-center">
                <span class="text-gray-600 dark:text-gray-400 underline decoration-dotted cursor-help">{{ flow.smsSent }}</span>
              </div>
      ...
      <!-- Analytics Section -->
      <template v-if="activeSection === 'global-kpis' || activeSection === 'delivery-performance' || activeSection === 'return-intelligence' || activeSection === 'risk-zones' || activeSection === 'anomaly-detection' || activeSection === 'trends' || activeSection === 'reports'">
        <header class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4">
          <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-gray-900 dark:text-white">Analytiques</h1>
            <div class="flex items-center space-x-3">
              <select class="px-3 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm">
        ...
        <main class="flex-1 overflow-y-auto p-6">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div class="bg-white dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-800">
              <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total colis</p>
              <p class="text-3xl font-semibold text-gray-900 dark:text-white mt-2">{{ shipments.length }}</p>
              <p class="text-sm text-green-600 mt-1">+12% par rapport à la période précédente</p>
            </div>
            <div class="bg-white dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-800">
              <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Taux de livraison</p>
              <p class="text-3xl font-semibold text-gray-900 dark:text-white mt-2">94,5%</p>
              <p class="text-sm text-green-600 mt-1">+2,3% par rapport à la période précédente</p>
            </div>
            <div class="bg-white dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-800">
              <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Temps de transit moy.</p>
              <p class="text-3xl font-semibold text-gray-900 dark:text-white mt-2">3,2 jours</p>
              <p class="text-sm text-red-600 mt-1">+0,5 jours par rapport à la période précédente</p>
            </div>
            <div class="bg-white dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-800">
              <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Taux d'exception</p>
              <p class="text-3xl font-semibold text-gray-900 dark:text-white mt-2">2,1%</p>
              <p class="text-sm text-green-600 mt-1">-0,8% par rapport à la période précédente</p>
            </div>
          </div>

          <!-- Chart -->
          <div class="bg-white dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-800 mb-6">
            <h3 class="font-semibold text-gray-900 dark:text-white mb-6">Colis au fil du temps</h3>
            <div class="h-64 flex items-end space-x-2">
              <div v-for="(value, index) in chartData" :key="index" class="flex-1 flex flex-col items-center">
                <div
                  class="w-full bg-orange-500 rounded-t transition-all duration-300"
                  :style="{ height: value + '%' }"
                ></div>
          ...
          <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
            <div class="p-4 border-b border-gray-200 dark:border-gray-800">
              <h3 class="font-semibold text-gray-900 dark:text-white">Performance des transporteurs</h3>
            </div>
            <table class="w-full">
              <thead class="bg-gray-50 dark:bg-gray-800/50">
                <tr>
                  ...
                  <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Livrées</th>
                  <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Taux livraison</th>
                  <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Temps moy.</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                <tr v-for="carrier in carriers" :key="carrier.name" class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{{ carrier.deliveryRate }}%</td>
            ...
            <div class="flex items-start space-x-4">
              ...
              <div class="flex-1">
                ...
                <p class="text-sm text-amber-700 dark:text-amber-400 mt-1">
                  <strong>45% de vos retours</strong> sont dus à des clients absents. Considérez d'appeler les clients avant livraison ou proposer des créneaux de livraison flexibles pour réduire ce taux.
                </p>
              </div>
            </div>
          </div>
      ...
      <template v-if="activeSection === 'connected-carriers' || activeSection === 'add-carrier' || activeSection === 'api-status' || activeSection === 'carrier-performance' || activeSection === 'sla-metrics' || activeSection === 'carrier-issues'">
        <header class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4">
          <div class="flex items-center justify-between">
            ...
            <button @click="openAddCarrierModal" class="flex items-center space-x-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-sm font-medium transition-colors">
              <Plus class="w-4 h-4" />
      ...
      <template v-if="activeSection === 'page-templates' || activeSection === 'my-tracking-page' || activeSection === 'page-branding' || activeSection === 'page-analytics' || activeSection === 'failed-searches'">
        <header class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4">
          <div class="flex items-center justify-between">
            ...
            <button v-if="activeSection === 'page-templates'" class="flex items-center space-x-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-sm font-medium transition-colors">
              <Plus class="w-4 h-4" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div v-for="template in trackingPageTemplates" :key="template.id" class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden hover:shadow-lg transition-shadow cursor-pointer group">
                ...
                <div class="p-4">
                  ...
                  <div class="flex items-center space-x-2">
                    <button class="flex-1 px-3 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-sm font-medium transition-colors">
                      Utiliser
                    </button>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
              <div class="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                  ...
                  <button class="px-3 py-1.5 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-sm font-medium">
                    Voir la page
                  </button>
                </div>
              </div>
          ...
          <div v-if="activeSection === 'page-branding'">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
                ...
                <button class="mt-6 w-full px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-sm font-medium">
                ...
                <div class="space-y-4">
                  <div>
                    <div class="flex items-center justify-between mb-1">
                      <span class="text-sm text-gray-600 dark:text-gray-400">Lien direct (SMS)</span>
                  ...
                  <tr v-for="search in failedSearches" :key="search.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/30">
                    ...
                    <td class="px-4 py-3 text-right">
                      <div class="flex items-center justify-end space-x-2">
                        <button v-if="search.phone" class="p-1.5 text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded">
                          <PhoneIcon class="w-4 h-4" />
                        </button>
        ...
        <header class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
              ...
              <button class="flex items-center space-x-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-sm font-medium">
          ...
          <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 mb-6">
            ...
            <div class="p-6">
              ...
              <div class="mt-6 flex justify-end">
                <button class="flex items-center space-x-2 px-6 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-medium transition-colors">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 divide-y divide-gray-200 dark:divide-gray-800">
              <div class="p-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    <p class="font-medium text-gray-900 dark:text-white">Profil entreprise</p>
                    <p class="text-sm text-gray-500">Nom, adresse, logo</p>
                  </div>
                </div>
                <ChevronRight class="w-5 h-5 text-gray-400" />
              </div>
              <div @click="activeSection = 'users-roles'" class="p-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">
                <div class="flex items-center space-x-3">
                  <Users class="w-5 h-5 text-gray-500" />
                  <div>
                    <p class="font-medium text-gray-900 dark:text-white">Utilisateurs & Rôles</p>
                    <p class="text-sm text-gray-500">Gérer l'équipe et les permissions</p>
                  </div>
                </div>
                <ChevronRight class="w-5 h-5 text-gray-400" />
              </div>
    ...
    <Teleport to="body">
      <div v-if="showAddShipmentModal" class="fixed inset-0 z-50 flex items-center justify-center">
        <div class="absolute inset-0 bg-black/50" @click="showAddShipmentModal = false"></div>
        <div class="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md p-6">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Ajouter un colis</h3>
            <button @click="showAddShipmentModal = false" class="p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg">
              <X class="w-5 h-5 text-gray-500" />
            </button>
          </div>
          <div class="space-y-4">
          ...
          <div class="flex justify-end space-x-3 mt-6">
            <button
              @click="showAddShipmentModal = false"
              class="px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800"
            >
              Annuler
            </button>
      ...
      <div v-if="showAddCarrierModal" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto py-4">
        ...
        <div class="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
          ...
          <div class="sticky bottom-0 bg-white dark:bg-gray-900 flex justify-end space-x-3 p-6 border-t border-gray-200 dark:border-gray-800">
            ...
            <button @click="saveCarrier" class="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-sm font-medium">
        ...
        <div class="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
          ...
          <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-800">
            ...
            <div class="flex items-center space-x-2">
              <button
                @click="printLabel"
                class="flex items-center space-x-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg text-sm font-medium"
              >
                <Printer class="w-4 h-4" />
                <span>Imprimer</span>
              </button>
...
const dailyTasksCategories = ref([
  {
    ...
    tasks: [
      { id: 1, title: 'Appeler Ahmed Ben Ali', description: 'YAL-2026-001234 · Tunis · 89 TND', completed: false, priority: 'normal', action: 'call', actionLabel: 'Appeler', actionIcon: markRaw(PhoneIcon), completedAt: '' },
      { id: 2, title: 'Appeler Fatma Trabelsi', description: 'ZR-2026-005678 · Sfax · 145 TND', completed: true, priority: 'normal', action: 'call', actionLabel: 'Appeler', actionIcon: markRaw(PhoneIcon), completedAt: '09:45' },
      { id: 3, title: 'Appeler Mohamed Sassi', description: 'MAY-2026-009012 · Sousse · 67 TND - Historique retours', completed: false, priority: 'high', action: 'call', actionLabel: 'Appeler', actionIcon: markRaw(PhoneIcon), completedAt: '' },
      { id: 4, title: 'Rappeler Karim Hamdi', description: 'Pas de réponse ce matin - réessayer après 17h', completed: false, priority: 'normal', action: 'call', actionLabel: 'Appeler', actionIcon: markRaw(PhoneIcon), completedAt: '' },
      { id: 5, title: 'Appeler Leila Mrad', description: 'ZR-2026-002345 · Bizerte · 178 TND', completed: true, priority: 'normal', action: 'call', actionLabel: 'Appeler', actionIcon: markRaw(PhoneIcon), completedAt: '10:30' }
    ]
  },
  {
    id: 'orders',
  ...
  },
  {
    ...
    tasks: [
      { id: 17, title: 'Contacter Hichem Mansour', description: '3 tentatives échouées - risque retour', completed: false, priority: 'high', action: 'call', actionLabel: 'Appeler', actionIcon: markRaw(PhoneIcon), completedAt: '' },
...
const delayedShipments = ref([
  { id: 1, tracking: 'YAL-2026-000123', client: 'Hatem Mejri', destination: 'Gabès', delayDays: 5, carrier: 'Yalidine', phone: '+216 98 123 456' },
  { id: 2, tracking: 'ZR-2026-000456', client: 'Rim Ayari', destination: 'Tataouine', delayDays: 4, carrier: 'ZR Express', phone: '+216 55 789 012' },
  { id: 3, tracking: 'MAY-2026-000789', client: 'Youssef Gharbi', destination: 'Tozeur', delayDays: 3, carrier: 'Maystro', phone: '+216 22 345 678' },
  { id: 4, tracking: 'YAL-2026-001012', client: 'Ines Lahmar', destination: 'Kasserine', delayDays: 2, carrier: 'Yalidine', phone: '+216 97 901 234' },
  { id: 5, tracking: 'ECO-2026-001345', client: 'Walid Ben Salah', destination: 'Gafsa', delayDays: 2, carrier: 'Ecotrack', phone: '+216 50 567 890' },
  { id: 6, tracking: 'ZR-2026-001678', client: 'Amira Bouzid', destination: 'Médenine', delayDays: 1, carrier: 'ZR Express', phone: '+216 24 123 456' },
  { id: 7, tracking: 'MAY-2026-001901', client: 'Sofiane Triki', destination: 'Kébili', delayDays: 1, carrier: 'Maystro', phone: '+216 92 789 012' }
])
...
// Computed filtered delayed shipments
const filteredDelayedShipments = computed(() => {
  if (delayedFilter.value === 'all') return delayedShipments.value
  if (delayedFilter.value === '24h') return delayedShipments.value.filter(s => s.delayDays === 1)
  if (delayedFilter.value === '48h') return delayedShipments.value.filter(s => s.delayDays >= 2 && s.delayDays <= 3)
  if (delayedFilter.value === 'critical') return delayedShipments.value.filter(s => s.delayDays > 3)
  return delayedShipments.value
})
...
const teamMembers = ref([
  {
    id: 1,
    name: 'Fawzy OTHMAN',
    email: 'fawzyothman92@gmail.com',
    initials: 'F',
    avatarColor: 'bg-green-400',
    lastLogin: 'Jan 27, 2026 at 12:40 am',
    role: 'Owner'
  }
])
...
const reportAnalytics = reactive({
  ...
  reasonsBreakdown: [
    { name: 'Client absent', count: 27, percentage: 45, suggestion: 'Appelez les clients avant livraison pour confirmer leur disponibilité' },
  ...
  productImpact: 20,
  processAdvice: 'Implémentez un appel de confirmation avant livraison pour les commandes > 100 DT',
  processImpact: 25
})
